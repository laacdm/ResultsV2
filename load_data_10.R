...
summaryParts <- function(data_tab, variable) {
  part <- select(data_tab, c(CATEGORY, CONTEXT, PASSED, FAILED, NOT_APPLICABLE, IS_ERROR)) %>%
    mutate(CATEGORY = as.factor(CATEGORY)) %>%
    mutate(CONTEXT = as.factor(CONTEXT)) %>%
    group_by(CATEGORY, CONTEXT) %>%
    dplyr::summarise("sum" := sum({{variable}})) %>%
    spread(CONTEXT, sum) %>%
    ungroup() %>%
...
data1 <- mutate(data1, Validation.PctPass = (Validation.Pass / (Validation.Pass + Validation.Fail))*100) %>%
  mutate(Verification.PctPass = (Verification.Pass / (Verification.Pass + Verification.Fail))*100) %>%
  mutate(Total.PctPass = (Total.Pass / (Total.Pass + Total.Fail))*100) %>%
  adorn_totals()

data1$Validation.PctPass[4] = (data1$Validation.Pass[4] / (data1$Validation.Pass[4] + data1$Validation.Fail[4]))*100
data1$Verification.PctPass[4] = (data1$Verification.Pass[4] / (data1$Verification.Pass[4] + data1$Verification.Fail[4]))*100
data1$Total.PctPass[4] = (data1$Total.Pass[4] / (data1$Total.Pass[4] + data1$Total.Fail[4]))*100
...
data2 <- mutate(dataT, CATEGORY = factor(CATEGORY)) %>%
  ...
  replace_na(list(NUM_VIOLATED_ROWS = 0, PCT_VIOLATED_ROWS = 0, FAILED = 0, IS_ERROR = 0, NOT_APPLICABLE = 0, THRESHOLD_VALUE = 0)) %>%
  ...
  mutate(CHECK_LEVEL = recode_factor(CHECK_LEVEL,  TABLE = "Table", FIELD = "Field", CONCEPT = "Concept")) %>%
  mutate(CHECK_STATUS = recode(CHECK_STATUS, `1` = "Pass", `2` = "Fail", `3` = "Is Error", `4` = "Not Applicable")) %>%
  select(!c(IS_ERROR, NOT_APPLICABLE, PASSED, EXECUTION_TIME, SQL_FILE, 
            CONCEPT_ID, UNIT_CONCEPT_ID, CDM_FIELD_NAME)) %>%
  relocate(CDM_TABLE_NAME, CHECK_NAME, checkId, CHECK_DESCRIPTION, CHECK_STATUS,
           CATEGORY, SUBCATEGORY, CONTEXT, CHECK_LEVEL, FAILED, CHECKS,
           PCT_PASSED, NUM_VIOLATED_ROWS, NUM_DENOMINATOR_ROWS, PCT_VIOLATED_ROWS, 
           THRESHOLD_VALUE)
